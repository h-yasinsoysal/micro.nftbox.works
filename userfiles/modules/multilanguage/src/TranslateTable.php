...
class TranslateTable
{
    ...
    public function getTranslate($data)
    {
        ...
        $translates = \MicroweberPackages\Multilanguage\Repositories\MultilanguageTranslateRepository::getTranslationsByRelTypeAndLocale($this->relType, $this->getCurrentLocale());

        if ($translates) {
            foreach ($translates->toArray() as $translate_item) {
